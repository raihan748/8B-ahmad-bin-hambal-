...
router.post('/gallery', requireOwner, upload.single('image'), (req, res) => {
    if (!req.file) return res.json({ success: false, message: 'No file uploaded.' });
    const db = getDb();
    const result = db.prepare(
        `INSERT INTO gallery (filename, caption, uploaded_by) VALUES (?, ?, ?)`
    ).run(req.file.filename, req.body.caption || '', req.session.user.username);
    logAction(req.session.user.username, 'UPLOAD', `Gallery: ${req.file.filename}`, req.body.caption || '');
    db.close();
...
router.delete('/gallery/:id', requireOwner, (req, res) => {
    const db = getDb();
    const img = db.prepare('SELECT filename FROM gallery WHERE id = ?').get(req.params.id);
    if (img) {
